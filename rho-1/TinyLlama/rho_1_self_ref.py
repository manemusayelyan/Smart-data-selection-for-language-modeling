...
class TinyLlamaCPTTrainer:
    ...
    def train(self):
        ...
        selected_num = torch.zeros(1, device=self.device)
        selected_denom = torch.zeros(1, device=self.device)
        ...
        while self.iter_num < max_iters:
            ...
            with sync_ctx:
                ...
                if args.selection_method == "self_reference_intersection":
                    low_ref_loss_mask = top_ratio_mask(
                        -ref_token_loss,
                        args.rm_loss_select_ratio,
                    )
                    low_ref_entropy_mask = top_ratio_mask(
                        -ref_entropy,
                        args.rm_entropy_select_ratio,
                    )
                    final_mask = low_ref_loss_mask & low_ref_entropy_mask
                elif args.selection_method == "rho_excess":
                    excess_loss = token_loss.detach() - ref_token_loss
                    final_mask = top_ratio_mask(
                        excess_loss,
                        args.excess_loss_topk_ratio,
                    )
                else:
                ...
                selected_num += selected_tokens.detach()
                selected_denom += n_tokens
            ...
            if not is_accumulating:
                ...
                if self.step_count % args.log_interval == 0:
                    ...
                    dist.all_reduce(selected_num, op=dist.ReduceOp.SUM)
                    dist.all_reduce(selected_denom, op=dist.ReduceOp.SUM)

                    loss_avg = loss_num / loss_denom.clamp_min(1.0)
                    selected_ratio = selected_num / selected_denom.clamp_min(1.0)
                    ...
                    if self.is_main:
                        ...
                        print(
                            ...
                            f"selected {selected_ratio.item():.3f} | "
                            f"lr {lr:.2e} | "
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--reference_checkpoint_dir", type=str, default=None)
    p.add_argument("--train_data_dir", type=str, default="/mnt/weka/mmusayelyan/data/prep_datasets/owm")
    p.add_argument("--out_dir", type=str, default="/mnt/weka/mmusayelyan/tinyllama_ct")
    ...
    p.add_argument("--stable_steps", type=int, default=19101)
    p.add_argument("--max_steps", type=int, default=23250)
    ...
    p.add_argument(
        "--selection_method",
        type=str,
        default="self_reference_intersection",
        choices=("self_reference_intersection", "rho_excess"),
    )
    p.add_argument(
        "--rm_loss_select_ratio",
        "--rm_loss_topk_ratio",
        dest="rm_loss_select_ratio",
        type=float,
        default=0.7,
    )
    p.add_argument(
        "--rm_entropy_select_ratio",
        "--rm_entropy_topk_ratio",
        dest="rm_entropy_select_ratio",
        type=float,
        default=0.7,
    )
    p.add_argument(
        "--excess_loss_topk_ratio",
        type=float,
        default=0.6,
    )
